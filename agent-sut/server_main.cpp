...
static json ElemToJsonLite(const UiaElem& e, int idx, HWND fgRoot) {
    ...
    json j = {
        {"idx", idx},
        {"name", ws2utf8(e.name)},
        {"automationId", ws2utf8(e.automationId)}, // Eğer uia_utils'de çektiysen kalsın
        {"value", ws2utf8(e.value)},               // <--- BU SATIRI EKLE
...
static json make_state(UiaSession& uia, std::vector<unsigned char>* raw_png_out, bool want_screenshot = true){
    ...
    auto elems = uia.snapshot_filtered(2500);  // daha kucuk & filtreli

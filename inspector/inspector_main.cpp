...
// Configuration - ODS_HOST env var overrides default (localhost)
...
// ============================================================================
struct JvmElement {
    std::string id;         // e.g. "W0.100.0.2"
    std::string className;  // e.g. "JButton"
    std::string text;       // e.g. "Submit"
...
// ============================================================================
// GET JVM ELEMENT AT POINT
// Like WinDriver: flatten all elements first, then find by coordinate
// ============================================================================

// Helper struct for flattened JVM element
struct FlatJvmElement {
    std::string id;
...
// Flatten JVM elements recursively (like controller's _flatten_jvm_elements)
void FlattenJvmElements(const json& node, std::vector<FlatJvmElement>& result) {
    // Extract element if it has text and geometry
    ...
    if (!text.empty() && 
        ...
        double w = node["width"].get<double>();
        double h = node["height"].get<double>();
        
        if (w > 0 && h > 0) {
            FlatJvmElement elem;
            elem.id = node.value("id", "");
            elem.className = node.value("class", "");
            elem.text = text;
            elem.x = x;
            elem.y = y;
            elem.width = w;
    ...
    if (node.contains("children") && node["children"].is_array()) {
        for (const auto& child : node["children"]) {
            FlattenJvmElements(child, result);
        }
    }
}

JvmElement GetJvmElementAtPoint(int x, int y) {
    JvmElement result;
    
    // Get window PID at this point
    POINT pt = { x, y };
    ...
    // Java's getLocationOnScreen() returns coordinates at 100% scale
    // but Windows reports click coordinates at the current DPI scale
    HDC hdc = GetDC(root);
    double dpiScale = 1.0;
    if (hdc) {
        int dpiX = GetDeviceCaps(hdc, LOGPIXELSX);
        dpiScale = dpiX / 96.0;  // 96 DPI = 100% scale
        ReleaseDC(root, hdc);
    }
    
    // Convert Windows screen coordinates to Java's 100% scale coordinates
    int wx = static_cast<int>(x / dpiScale);
    int wy = static_cast<int>(y / dpiScale);
    
    // Capture JVM snapshot for this PID
    auto snapshot = jvm_bridge::CaptureSnapshot(pid);
    if (!snapshot.success || snapshot.snapshotJson.empty()) {
        result.error = snapshot.errorMessage;
        return result;
    }
    
    try {
        json data = json::parse(snapshot.snapshotJson);
        
        // STEP 1: Flatten all elements (like controller does)
        std::vector<FlatJvmElement> flatElements;
        if (data.contains("elements") && data["elements"].is_array()) {
            for (const auto& elem : data["elements"]) {
                FlattenJvmElements(elem, flatElements);
            }
        }
        
        // STEP 2: Find element at coordinate (like WinDriver)
        // Priority: smallest area element containing click point
        FlatJvmElement* bestMatch = nullptr;
        double bestArea = std::numeric_limits<double>::max();
        
        for (auto& elem : flatElements) {
            // Skip very large elements (like JFrame window title)
            double area = elem.width * elem.height;
            if (area > 100000) continue;
            
            // Check if window-relative click point is inside this element
            if (wx >= elem.x && wx < elem.x + elem.width &&
                wy >= elem.y && wy < elem.y + elem.height) {
                // Prefer smaller area (more specific element)
                if (area < bestArea) {
                    bestArea = area;
                    bestMatch = &elem;
                }
        ...
        // If no containing element found, find nearest
        if (!bestMatch) {
            ...
            for (auto& elem : flatElements) {
                ...
                // Calculate distance to bounding box using window-relative coords
                double dx = 0, dy = 0;
                if (wx < elem.x) dx = elem.x - wx;
                else if (wx > elem.x + elem.width) dx = wx - (elem.x + elem.width);
                if (wy < elem.y) dy = elem.y - wy;
                else if (wy > elem.y + elem.height) dy = wy - (elem.y + elem.height);
                
                double distance = std::sqrt(dx*dx + dy*dy);
                
                // Prefer closer, or smaller area if same distance
                if (distance < bestDistance - 3 ||
                    (std::abs(distance - bestDistance) <= 3 && area < bestArea)) {
        ...
        if (bestMatch) {
            result.id = bestMatch->id;
            result.className = bestMatch->className;
            result.text = bestMatch->text;
            result.found = true;
        }
...
void HandleF4Press(int mouse_x, int mouse_y, bool removeHover = false) {
    ...
    try {
        ...
        // ============================================
        // JVM (Swing/JavaFX) Element Info
        // ============================================
        std::cout << "==========================================\n";
        std::cout << "[JVM]\n";
        JvmElement jvmElem = GetJvmElementAtPoint(mouse_x, mouse_y);
        if (jvmElem.found) {
            std::cout << "  id    : \"" << jvmElem.id << "\"\n";
            std::cout << "  class : \"" << jvmElem.className << "\"\n";
            std::cout << "  text  : \"" << jvmElem.text << "\"\n";
        } else {
            if (!jvmElem.error.empty()) {
                std::cout << "  [!] " << jvmElem.error << "\n";
            } else {
                std::cout << "  [!] No JVM element found at this point\n";
            }
        }
        std::cout << "==========================================\n\n";
...
int main() {
    ...
    std::cout << "=== INSPECTOR (ODS + WIN + JVM) ===\n";
    ...
    std::cout << "JVM: Active (JVM_DIR env or ./jvm)\n";
    std::cout << "===================================\n\n";
